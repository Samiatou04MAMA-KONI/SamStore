...
import { jeuxCategories } from '../../data/jeuxdata';
...
const Jeux = () => {
  ...
  const handleAddToCart = (product, event) => {
    ...
    const cartProduct = {
      ...
      image: product.image,
      quantity: 1,
      category: product.category || 'jeux',
  ...
  const productsToShow = selectedSubcategory 
    ? jeuxCategories.subcategories.find(sub => sub.id === selectedSubcategory)?.products || []
    : jeuxCategories.subcategories.flatMap(sub => sub.products);
        ...
        <div className="container">
          ...
          <div className="subcategories-grid">
            ...
            >
              ...
              <p>{jeuxCategories.subcategories.flatMap(sub => sub.products).length} produits</p>
            </div>

            {jeuxCategories.subcategories.map(subcategory => (
  <div 
    key={subcategory.id}
      ...
      <section ref={productsSectionRef} className="products-section">
        <div className="container">
          <div className="section-header">
            <h2>
              {selectedSubcategory 
                ? `${jeuxCategories.subcategories.find(sub => sub.id === selectedSubcategory)?.name}`
                : 'Tous les Produits Jeux & Jouets'
              }
