...
import { useCart } from '../../context/contextPanier'; // Ajoutez cette ligne
...
const Electronic = () => {
  ...
  const productsToShow = selectedSubcategory 
    ? electronicsCategories.subcategories.find(sub => sub.id === selectedSubcategory)?.products || []
    : electronicsCategories.subcategories.flatMap(sub => sub.products);
    ...
    <div className="electronics-page">
      ...
      <section className="electronics-hero">
        <div className="hero-content">
          <h1>📱 {electronicsCategories.name}</h1>
        ...
        <div className="container">
          ...
          <div className="subcategories-grid">
            ...
            >
              ...
              <p>{electronicsCategories.subcategories.flatMap(sub => sub.products).length} produits</p>
            </div>

            {electronicsCategories.subcategories.map(subcategory => (
  <div 
    key={subcategory.id}
      ...
      <section ref={productsSectionRef} className="products-section">
        <div className="container">
          <div className="section-header">
            <h2>
              {selectedSubcategory 
                ? `${electronicsCategories.subcategories.find(sub => sub.id === selectedSubcategory)?.name}`
        ...
        <div className="product-pricing-simple">
          {product.originalPrice > (product.price || product.currentPrice) && (
            <span className="original-price-simple">
              {(product.originalPrice || 0).toLocaleString()} FCFA
            </span>
          )}
          <span className="current-price-simple">
            {(product.price || product.currentPrice || 0).toLocaleString()} FCFA
          </span>
        </div>
