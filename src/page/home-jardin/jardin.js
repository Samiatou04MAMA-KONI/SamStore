...
import { jardinCategories } from '../../data/jardindata';
...
const Jardin = () => {
  ...
  const handleAddToCart = (product, event) => {
    ...
    const cartProduct = {
      ...
      image: product.image,
      quantity: 1,
      category: product.category || 'jardin',
  ...
  const productsToShow = selectedSubcategory 
    ? jardinCategories.subcategories.find(sub => sub.id === selectedSubcategory)?.products || []
    : jardinCategories.subcategories.flatMap(sub => sub.products);
        ...
        <div className="container">
          ...
          <div className="subcategories-grid">
            ...
            >
              ...
              <p>{jardinCategories.subcategories.flatMap(sub => sub.products).length} produits</p>
            </div>

            {jardinCategories.subcategories.map(subcategory => (
  <div 
    key={subcategory.id}
      ...
      <section ref={productsSectionRef} className="products-section">
        <div className="container">
          <div className="section-header">
            <h2>
              {selectedSubcategory 
                ? `${jardinCategories.subcategories.find(sub => sub.id === selectedSubcategory)?.name}`
                : 'Tous les Produits Maison & Jardin'
              }
